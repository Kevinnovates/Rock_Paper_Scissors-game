...
while playAgain != "n":
    ...
    if (
        compChoice == "Rock" and userInput.capitalize() == "Rock"
    ...
    if (
        compChoice == "Rock" and userInput.capitalize() == "Paper"
    ...
    while playAgain not in (
        "y",
        "n",
        "Y",
        "N",
    ):  # check if player input an existing option
        playAgain = input(
            "That was not an option. Type y to play again or n to quit.\n"
        )

    if playAgain in ("y", "Y"):
        userInput = input("Rock, Paper, or Scissors?\n")  # ask player for input
        while (
            userInput.capitalize() not in choices
        ):  # check if player input an existing option
            userInput = input("That was not an option. Please try again.\n")
